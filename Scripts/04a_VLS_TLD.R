...
df_VL<-df %>%
  ...
  group_by(fiscal_year,operatingunit,psnuuid,psnu,indicator,otherdisaggregate) %>%
...
map_VLS<-terrain_map(countries = "Nigeria", terr_path = dir_terr, mask = TRUE) +
  geom_sf(data = nga_geo %>% filter(period=="FY20Q2" & !is.na(VLS)),
          ...
          # aes(fill=factor(VLS_cat,levels=c("Less than 80%","80-89%","Greater than 90%")), alpha=.85), lwd = .2, color = grey40k) +
  # geom_sf(data = nga1, fill = NA, lwd = .2, color = grey30k) +
  # scale_fill_manual(values=c("#d7191c", "#ffeda0", "#008837"))+
...
scatter<-df_VL %>%
    ...
    ggplot(aes(
        x=TLD_MOT,
        y=VLS))+
    geom_point(color=grey50k,size=4,alpha=0.7)+
...
ggsave(here("Graphics", "Nigeria_VLS_TLD_TLE_Ratio.png"),
