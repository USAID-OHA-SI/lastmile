##  PROJECT: Q3 Review
##  AUTHOR:  B.Kagniniwa, G.Sarfaty & T.Essa | USAID
...
proxy_ovc_cov <- df %>%
  ...
  filter(period == "FY20Q2", !is.na(proxy_coverage))

#proxy_ovc_cov <- extract_ovc_coverage(df)
...
# Map
zam_map_cov20 <- terrain_map(countries = "Zambia",
                             terr_path = dir_terr,
                             mask = TRUE) +
  geom_sf(data = zam_geo %>% filter(proxy_coverage > 0),
          aes(fill = proxy_coverage), lwd = .2, color = grey10k) +
