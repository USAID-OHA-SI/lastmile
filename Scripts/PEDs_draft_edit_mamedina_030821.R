...
cntry_peds <- peds_psnu %>%
  ...
  ungroup() %>%
  group_by(operatingunit, countryname, snu1, snu1uid, #mech_name, mech_code,
  ...
  mutate(primepartner = paste0(fundingagency, "-",
                        ...
                        "%)"))
        #nonPEP =case_when(primeshare is.na() ~ "Less than 85%",
                         # TPT >=.85 ~ "85% or Greater"

       # ))

#need to be able to create 1 and 0 values for the NON PEPFAR SNUS
#USAID and CDC - de-dup removed filter !dedup
#Change to TBD by changing to new dataset
#National by sub-national () #NAT_SUBNAT
#use BT psnu IM FY19-21


cntry_peds %>% distinct(primepartner)
...
ggsave(here("~/Github/training/Graphics/map1_peds", "MAP1_Burundi_TX_CURR by IP.png"),
       scale = 1.2, dpi = 310, width = 10, height = 7, units = "in")

#write.csv(cntry_peds,"C:\\Users\\STAR\\Documents\\Github\\peds_txcurr_0308.csv", row.names = FALSE)
...
#  if (!is.null(country)) {
 #   df_geo <- df_geo %>%
  ...
  basemap <- terrain_map(countries = ou,
                         terr = si_path(type = "path_raster"),
                         mask = TRUE) +
                         geom_sf(data = peds_geo %>% filter(is.na(share)), aes(fill=#value i come up with),
                                 lwd = .2, color = grey50k)+
                         si_style_map()
...
ggsave(here("~/Github/training/Graphics/map1_peds", "MAP1TEST_Nigeria_TX_CURR by IP.png"),
       scale = 1.2, dpi = 310, width = 10, height = 7, units = "in")
