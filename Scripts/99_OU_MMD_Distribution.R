...
# DATA MMD Distribution ----
  ...
  df_mmd_trend <- df_mmd %>%
    ...
    ungroup() %>%
    select(-c(value, mmd_total)) %>%
    rename(mmd_len = otherdisaggregate) %>%
    pivot_wider(names_from = period, values_from = mmd_share)
...
# VIZ ----
  ...
  df_mmd_share %>%
    ...
    geom_text(aes(y = 0.03, color = share_color), size = 3) +
    scale_x_reordered() +
  ...
  si_save(
    filename = file.path(
      dir_graphics,
      glue("{rep_pds %>% last()} - {str_to_upper(cntry)} - MMD Dispensing plot - {format(Sys.Date(), '%Y%m%d')}.png")),
    plot = last_plot(),
    width = 9.54,
    height = 5,
    scale = 1.4)
  ...
  si_save(
    filename = file.path(
      dir_graphics,
      glue("{rep_pds %>% last()} - {str_to_upper(cntry)} - MMD Trend plot - {format(Sys.Date(), '%Y%m%d')}.png")),
    plot = last_plot(),
    width = 9.54,
    height = 5,
    scale = 1.4)
  ...
  si_save(
    filename = file.path(
      dir_graphics,
      glue("{rep_pds %>% last()} - {str_to_upper(cntry)} - MMD Trend plot - {format(Sys.Date(), '%Y%m%d')}.svg")),
    plot = last_plot(),
    width = 9.54,
    height = 5,
    scale = 1.4)
  ...
  c("3+", "6+") %>%
    map(function(mmd_len) {
      ...
      si_save(
        filename = file.path(
          dir_graphics,
          glue("{rep_pds %>% last()} - {str_to_upper(cntry)} - MMD{mmd_len} Scaling map - {format(Sys.Date(), '%Y%m%d')}.svg")),
        plot = map,
        width = 9.54,
        height = 5,
        scale = 1.4)
