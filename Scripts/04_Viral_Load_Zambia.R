...
# MER Data - Global MSD 20200918
file_psnu_im <- (list.files(path = dir_merdata,
                            pattern = "Structured_.*_PSNU_IM.*_20200918_.*.zip",
                            recursive = TRUE,
                            full.names = TRUE))
...
zam_map_all <- terrain_map(countries = "Zambia",
                           terr_path = dir_terr,
                           mask = TRUE) +
  geom_sf(data = zam_geo_all %>% filter(!is.na(Not_Cov)),
          aes(fill = VLC), lwd = .2, color = grey10k) +
  ...
  ggtitle(label = "", subtitle = "Viral Load Coverage - All Ages")



vls_map_all <- terrain_map(countries = "Zambia",
                           terr_path = dir_terr,
                           mask = TRUE) +
  geom_sf(data = zam_geo_all %>% filter(!is.na(VLS)),
...
# VIZ - UNDER 15 -------------------------------------------------------------------------------------------
zam_map_u15 <- terrain_map(countries = "Zambia",
                           terr_path = dir_terr,
                           mask = TRUE) +
  geom_sf(data = zam_geo_u15 %>% filter(!is.na(Not_Cov)),
          aes(fill = VLC), lwd = .2, color = grey10k) +
  geom_sf(data = zam1, fill = NA, lwd = .2, color = grey30k) +
  scale_fill_viridis_c(option = "magma", alpha = 0.9, direction = -1,
    breaks = c(0, .25, .50, .75, 1.00),
...
zam_vls_mapu15 <- terrain_map(countries = "Zambia",
                              terr_path = dir_terr, mask = TRUE) +
  geom_sf(data = zam_geo_u15 %>% filter(!is.na(VLS)),
          aes(fill = VLS), lwd = .2, color = grey10k) +
  geom_sf(data = zam1, fill = NA, lwd = .2, color = grey30k) +
  scale_fill_viridis_c(option = "viridis", alpha = 0.9, direction = -1,
    breaks = c(0, .25, .50, .75, 1.00),
